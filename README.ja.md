...
device_uuidは`localStorage`に保存され、かんたん認証がこの端末を「見覚えのある端末」と認識するための鍵になっている。iOS/iPadOSでは、同一オリジンであっても「ホーム画面に追加」したPWAはSafari本体とは別の独立した`localStorage`領域を持つ。そのためSafariで登録済みのユーザーがそのままホーム画面に追加すると、device_uuidが一切無い状態でPWAが開き、サインインし直せない状態になる。

`config('easy-auth.pwa_resync_path')`(`.env`の`EASY_AUTH_PWA_RESYNC_PATH`)は、アプリ側が選んだ推測不可能なパス(長いランダム文字列。このパッケージの公開ソースに書かれた値は「推測不可能」にはなり得ないため、パッケージ側にデフォルト値は用意していない)に`GET`ルートを登録する。このルートはパスキー認証を試み、結果にかかわらず黙ってアプリへリダイレクトする。認証に成功した場合、その端末に既に紐づいている**既存の**device_uuidをこのブラウジングコンテキストの`localStorage`に再保存するだけで、新しいデバイスやパスキーを作成することはない。PWAの`manifest.json`の`start_url`をこのパスに向けておけば、ホーム画面アイコンから開いた瞬間にこの復旧処理が自動的に(ゲスト画面が見える前に)1回だけ走る。未設定(デフォルト)の場合、ルート自体が登録されず機能は完全に無効のままとなる。

これは既存のdevice_uuid**不一致**時の処理(`SignInController`による強制ログアウト+アカウント削除リンクのメール送信)には一切手を加えない。今回対応するのはUUIDが**存在しない**場合のみで、しかもこの推測不可能なURLに到達したことをそのブラウジングコンテキストが既に証明している場合に限る。パスワードフォールバックの端末は再検証すべきパスキーを持たないため対象外とし、ゲストとしてそのままアプリへ進む。

...
- 登録時に`authenticatorAttachment`をplatformに強制し、選択ダイアログ自体からクロスプラットフォーム認証器・パスワードマネージャーのみの選択肢を除外する機能を`config('easy-auth.force_platform_authenticator')`(`.env`の`EASY_AUTH_FORCE_PLATFORM_AUTHENTICATOR`)で用意している。デフォルトoff。かつてWindows Hello側の不具合(登録時に`user.name`/`displayName`が空文字になり、以後のサインインが失敗する既知のバグ、修正済み)で「Helloという選択肢自体が実質使えない」状態のまま強制すると、認証器が一つも選べず登録不能になる実機事象があり実装を見送っていた経緯がある。当該バグの修正後に改めて有効化して実機検証する想定。
